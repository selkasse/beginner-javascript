// Use Map, Filter, and Reduce to get all instances of each letter and number

const allText = `Skip to main content
Select language
Skip to search
Technologies▼
References & Guides▼
...
SafariFull support4.1
WebView AndroidFull support≤37
Chrome AndroidFull support18
Firefox AndroidFull support4
Opera AndroidFull supportYes
Safari iOSFull support4
Samsung Internet AndroidFull support1.0
nodejsFull supportYes
...
function cleanUpMatch(char) {
    // a-z, A-Z, 0-9
    return char.match(/[a-zA-Z0-9]/);
}
function standardizeCases(char) {
    return char.toUpperCase();
}

function countChars(totals, char) {
    totals[char] = totals[char] + 1 || 1;
    return totals;
}

// Get the entire string into an array of characters
const allChars = allText.split('');

// Remove newline characters like '↵', which 'filter' does not catch
// eslint-disable-next-line no-plusplus
// for (let i = 0; i < allChars.length; ++i) allChars[i] = allChars[i].replace(/(\r\n|\n|\r)/gm, '');

// Filter all of the characters that are not letters or numbers
const cleanedChars = allChars.filter(cleanUpMatch);

// Put the characters into a Set to output to easily see if there are any characters the filter missed
const cleanedUnique = new Set(cleanedChars);

// Since we are counting capital and lowercase letters as the same letter,
// We can use a map so that all of the characters are the same case
const uniformCaseChars = cleanedChars.map(standardizeCases);

// Put the characters into a Set to output to easily see if there are any characters the map missed
const uniformUnique = new Set(uniformCaseChars);

// Now that we have all of the characters we are going to count, we can count their totals with reduce
// We start with an empty Object because our final output will be in this format:
// {
//   A: 5,
//   B: 2,
//   C: 1,
//   5: 2
// }
const charCounts = uniformCaseChars.reduce(countChars, {});

function sortByValue(a, b) {
    // Use index 1 because each element looks like:
    // ["character", count], and we want the count
    return a[1] - b[1];
}
// Now that we have the counts of each character,
// we want to sort them by most / least used character
const sortedResult = Object.entries(charCounts).sort(sortByValue);
