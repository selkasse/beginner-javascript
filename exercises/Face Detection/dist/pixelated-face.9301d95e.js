...
optionsInputs.forEach(input => input.addEventListener('input', handleOption)); // Write a function that will populate the user's video
...
async function populateVideo() {
  ...
  video.srcObject = stream; // wait until the stream has loaded in to play the video
...
function drawFace(face) {
  const {
    ...
    top,
    left
  } = face.boundingBox; // clear the canvas each time so that the boxes don't repeat

  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.strokeStyle = '#ffc600';
  ctx.lineWidth = 2; // ctx.strokeRect(left, top, width, height);
  // console.log(face);
...
function censor({
  ...
  face.x, face.y, face.width, face.height, // 4 draw args
  // x and y are where we start drawing
  face.x, face.y, // size of the shunken face (size x size) in px
  options.SIZE, options.SIZE); // draw the small face back on, but scale up
...
async function detect() {
  ...
  faces.forEach(censor); // ask the browser when the next animation frame is
...
},{}]},{},["../../../../../../../home/idev/.nvm/versions/node/v13.5.0/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","pixelated-face.js"], null)
